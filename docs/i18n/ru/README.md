...
## Талисман проекта «Dora 小途»

Янтарный кот-путешественник в шляпе исследователя, который тащит чемодан в наклейках и гонится за бумажным самолётиком, — «рождён» фреймворком **e-cat (одна кошка)**. Морда кота основана на [Twemoji](https://github.com/jdecked/twemoji) 1f431 (CC-BY 4.0) с изменениями; шляпа исследователя / чемодан / бумажный самолётик — оригинальные. Векторный исходник и полное описание — в [`docs/mascot.svg`](../../mascot.svg).

| Точка размещения | Форма |
| :--- | :--- |
| `docs/mascot.svg` | **Единственный векторный исходник** (в полный рост 512×512) |
| `apps/*/web/favicon.svg` | Иконка вкладки браузера (крупный план морды, различима даже при 16px; `favicon.png` 16px — запасной вариант для старых браузеров) |
...
| `apps/client/harmonyos/.../media/mascot.svg` | Внутри приложения HarmonyOS (`Image` нативно рендерит SVG; упрощённый мобильный вариант) |
| Каждый README / документация крейтов | Брендовое место в начале страницы |

> Чтобы изменить внешний вид, правьте только `docs/mascot.svg` — всё остальное производное: favicon — это вырезанный крупный план морды, а вариант для HarmonyOS убирает `<defs>`/градиенты и использует сплошные цвета под мобильные SVG-рендереры.
...
│   ├── client/
│   │   ├── flutter/       # Flutter: iOS / Android / Web / Desktop (i18n на 12+ языках; web/favicon.svg — иконка «Dora 小途»)
...
│   ├── mascot.svg         # Талисман «Dora 小途» (единственный векторный исходник; favicon и иконки платформ производны от него)
