...
<p align="center"><img src="../../mascot.svg" alt="Nannan 南南 — Open Travel талисман" width="180"></p>
...
Open Travel — это monorepo глобальной туристической платформы, построенной на **e-cat (одна кошка)** — Rust-микросервисном фреймворке уровня [go-kratos/kratos](https://github.com/go-kratos/kratos) v3 (v3.0.3 · 52 крейта) — для высокопроизводительного бэкенда, в сочетании с клиентами на Flutter и нативным клиентом HarmonyOS, обеспечивающими единый опыт бронирования путешествий для пользователей по всему миру.

| Измерение | Описание |
| :--- | :--- |
| **Бэкенд-фреймворк** | e-cat (Rust): HTTP/axum + gRPC/tonic, экосистема из 52 микросервисных крейтов |
| **Мультиплатформенные клиенты** | `apps/client/flutter` (iOS / Android / Web / Desktop), `apps/client/harmonyos` (HarmonyOS), `apps/admin` (консоль администратора Flutter Web) |
...
## Талисман проекта «Nannan 南南»

Дух компаса: круглый циферблат — это тело, на ободке **12 делений = 12+ языков**, над головой стоит стрелка, в правой руке — лупа для поиска пункта назначения, а слева вверху бумажный самолётик тянет за собой маршрут. Плоская геометрическая форма (чистые цветовые блоки + тонкие обводки, без градиентов и полупрозрачности); векторный исходник и полное описание — в [`docs/mascot.svg`](../../mascot.svg).

| Точка размещения | Форма |
| :--- | :--- |
| `docs/mascot.svg` | **Единственный векторный исходник** (в полный рост 512×512) |
| `apps/*/web/favicon.svg` | Иконка вкладки браузера (крупный план циферблата, различима даже при 16px; `favicon.png` 16px — запасной вариант для старых браузеров) |
| `apps/*/web/icons/Icon-*.png` | Иконки PWA / домашнего экрана 192·512 (включая вариант с безопасной зоной maskable) |
| `apps/*/assets/mascot.png` | Показ внутри приложения Flutter (страница входа админки, страница профиля клиента) |
| `apps/client/harmonyos/.../media/mascot.svg` | Внутри приложения HarmonyOS (`Image` нативно рендерит SVG; мобильный вариант без `opacity`) |
| Каждый README / документация крейтов | Брендовое место в начале страницы |

> Чтобы изменить внешний вид, правьте только `docs/mascot.svg` — всё остальное производное: favicon — это крупный план циферблата (без рук / лупы / ног / маршрута и других штрихов, которые расплываются при малом размере), а вариант для HarmonyOS преумножает полупрозрачные цвета в непрозрачные и убирает весь `opacity` под мобильные SVG-рендереры. Вся графика оригинальная, без сторонних материалов.
...
## Диаграмма проектирования архитектуры

![Диаграмма проектирования архитектуры](../../svg/ru/architecture.svg)

## Диаграмма проектирования функций

![Диаграмма проектирования функций](../../svg/ru/features.svg)

## Диаграмма структуры проекта

![Диаграмма структуры проекта](../../svg/ru/project.svg)

## Диаграмма жизненного цикла запроса

![Диаграмма жизненного цикла запроса](../../svg/ru/request-cycle.svg)
...
```
open-travel/
├── apps/                  # Мультиплатформенные клиенты и админ-панель
│   ├── client/
│   │   ├── flutter/       # Flutter: iOS / Android / Web / Desktop (i18n на 12+ языках; web/favicon.svg — иконка «Nannan 南南»)
│   │   └── harmonyos/     # Нативный клиент HarmonyOS
│   └── admin/             # Админ-панель на Flutter Web
├── e-cat/                 # e-cat фреймворк + бизнес-сервисы (единый Cargo workspace)
│   ├── ecat*/             # 52 крейта фреймворка ecat-*
│   ├── ecat/              # Основной крейт: фасад + бизнес-модули (src/business/) + входы сервисов (src/bin/, 9 сервисов)
│   ├── config/            # Примеры конфигурации фреймворка
│   ├── examples/          # Примеры проектов фреймворка
│   └── CHANGELOG.md       # Журнал изменений фреймворка + проекта
├── docs/                  # Техническая документация
│   ├── api.md             # Справочник API (эндпоинты, аутентификация, ограничение запросов)
│   ├── mascot.svg         # Талисман «Nannan 南南» (единственный векторный исходник; favicon и иконки платформ производны от него)
│   ├── svg/               # Схемы архитектуры / функций / жизненного цикла / безопасности / структуры (с переводами на 12 языков)
│   ├── i18n/              # README на 12 языках
│   └── coin/              # QR-коды для донатов
├── config/                # Конфигурация окружения и развёртывания (nginx.conf, docker-compose.yml, schema.sql)
├── scripts/               # Установка / развёртывание / проверка / нагрузочные тесты / CDN / начальные данные
├── .github/workflows/     # CI
└── README.md
```
...
Требования: Docker + Docker Compose (v2). (Rust toolchain нужен только для сборки из исходников.)
...
Скрипт автоматически: проверяет окружение → собирает и запускает все сервисы (MySQL / Redis / OpenSearch / Kafka / 9 микросервисов / шлюз Nginx) → инициализирует поисковые индексы → выполняет проверку здоровья, после чего выводит URL доступа и учётную запись администратора по умолчанию `admin@travel.local` / `Admin@123`.
