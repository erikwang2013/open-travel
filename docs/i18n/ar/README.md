...
<p align="center"><img src="../../mascot.svg" alt="Nannan 南南 — Open Travel تميمة" width="180"></p>
...
جنية بوصلة: القرص الدائري هو الجسد نفسه، وعلى الإطار **12 علامة = أكثر من 12 لغة**، وينتصب في أعلاه مؤشر واحد، وترفع اليد اليمنى عدسة مكبّرة للبحث عن الوجهة، بينما في الزاوية العلوية اليسرى تسحب طائرة ورقية خيط المسار. تصميم هندسي مسطّح (كتل بألوان صلبة + حدود رفيعة، بلا تدرجات ولا ألوان نصف شفافة)، والمصدر المتجهي والتفاصيل الكاملة في [`docs/mascot.svg`](../../mascot.svg).

| الموقع | الشكل |
| :--- | :--- |
| `docs/mascot.svg` | **المصدر المتجهي الوحيد** (هيئة كاملة 512×512) |
| `apps/*/web/favicon.svg` | أيقونة تبويب المتصفح (نسخة مقرّبة من القرص، تبقى مميزة عند 16px؛ `favicon.png` بديل 16px للمتصفحات القديمة) |
...
| `apps/client/harmonyos/.../media/mascot.svg` | داخل تطبيق HarmonyOS (عرض SVG أصلي عبر `Image`؛ نسخة للجوال بلا `opacity`) |
| كل ملفات README / وثائق crate | موضع العلامة أعلى الصفحة |

> لإعادة تصميم الشخصية عدّل `docs/mascot.svg` فقط، فالباقي كله مشتقّات: الـ favicon هو تقريب للقرص (بحذف الذراعين / العدسة المكبّرة / القدمين / خط المسار وغيرها من الخطوط التي تتلاشى في الأحجام الصغيرة)، ونسخة HarmonyOS تضرب الألوان نصف الشفافة مسبقًا لتصبح ألوانًا صلبة ثم تحذف كل `opacity` لتلائم عارض SVG على الجوال. جميع الرسومات أصلية، بلا أي اعتماد على مواد من طرف ثالث.
...
│   ├── mascot.svg         # التميمة «南南» (المصدر المتجهي الوحيد، ومنه تُشتق favicon وأيقونات كل منصة)
